...
import Ranking from 'app_components/commons/ranking';
...
const StatisticsRankingSection = (props: any) => {
  ...
  useEffect(() => {
    ...
    const getRankingNational = async () => {
      ...
      const newRankingAll = sortByValue(response?.response)?.map((item : any, idx : number) => {
        return{
          ...item,
          id : item?._id,
          position : idx + 1,
    ...
    const getLocationRanking = async () => {
      ...
      const newRankingDistrict = sortByValue(response?.response)?.map((item : any, idx : number) => {
        return{
          ...item,
          id : item?._id,
          position : idx + 1,
  ...
  const sortByValue = (rankingList : any[]) => {
    return rankingList.sort((a,b) => (a.value > b.value) ? -1 : ((b.value > a.value) ? 1 : 0))
  }
