...
const SectionsMatterLayout = (props: ISectionsMatterLayout) => {
  ...
  const renderItem = ({item}: any = {}) => {
    ...
    const disabled =
      completedIndexes.find(
        sectionNumber => Number(index) - 1 === Number(sectionNumber),
  ...
  const getSectionProgress = (_section: any) => {
    ...
    const questions_of_section =
      props.academicResourceData.config.questionsByConfiguration[_section.uuid];
    const statistics =
  ...
  return (
    <React.Fragment>
      <ScrollView>
        ...
        />
        {props.isSimulacrum &&
        props.infoSections.length === Object.keys(completedSections).length ? (
          <ButtonBlue
