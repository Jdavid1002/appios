...
import {View, ActivityIndicator, Alert, ScrollView, FlatList} from "react-native"
...
import LessonCard from 'app_components/commons/lesson/containers/lesson-card';
...
class LearningPath extends Component<any> {
  ...
  getStatisticsProgress = async (learning_data : any) => {
    ...
    for (const _stage of learning_data?.course_data?.learning_way) {
      _stage?.childs?.map((item : any) => item?.id).forEach((id : string) => {
        lessons_ids.push(id)
      })
    }
  ...
  renderEmpty = () => <CustomText> No hay información para mostrar </CustomText>;
  renderItem = ({item}: any = {}) => <LessonCard {...item} progress={this.state.lesson_statistics[item?.id]} matterId={this.props.route.params.matterId} stage={this?.state?.learning_path[this?.state?.current_stage]?.id} />;
  ...
  render() {
    ...
    if (this.state.no_learning_path === true) {
      return (
        <ScrollView style={[mainStyles.container]}>
          <CustomText style={{color: '#e94044', fontSize: 32, fontWeight: 'bold', marginTop:20, marginBottom: 20, textAlign: 'center' }}>¡Ruta de aprendizaje!</CustomText>
          <CustomText style={{color: '#061946', fontSize: 20, marginBottom: 20}}>No tienes lecciones asociadas a esta ruta de aprendizaje.</CustomText>
        </ScrollView>
      )
    }
    ...
    return (
      <ScrollView style={[mainStyles.container]}>
        <CustomText style={{color: '#e94044', fontSize: 32, fontWeight: 'bold', marginTop:20, marginBottom: 20, textAlign: 'center' }}>¡Ya tienes tu ruta!</CustomText>
        <CustomText style={{color: '#061946', marginBottom: 20}}>Se ha generado tu ruta aprendizaje. Para mayor comodidad hemos organizado las lecciones en grupos de 10.</CustomText>
        ...
        <QuestionNavigate
          ...
          text='Etapa'
          colorText='#061946'
          onPressNextItem={() => {this.goToStage(this.state.current_stage +1)}}
          onPressPrevItem={() => {this.goToStage(this.state.current_stage-1)}}
        />
