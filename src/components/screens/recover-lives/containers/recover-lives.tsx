  ...
  getQuestionsData = async () => {
    ...
    if (data.status === 'success') {
      ...
      for (var i in data.questions_to_recover) {
        let comment = data.questions_to_recover[i].answers.filter((a: any) => a.comment !== '');
        let data_q = {
          number: (parseInt(i)+1),
  ...
  render() {
    ...
    return (
      <ScrollView style={[mainStyles.container]}>
        ...
        <CustomText style={styles.text}>Puedes esperar
          <CustomText style={styles.textBold}> 15 minutos </CustomText>
          o mira las lecciones de tus respuestas erróneas, así tendrás
          <CustomText style={styles.textBold}> 3 nuevas oportunidades. </CustomText>
        </CustomText>
